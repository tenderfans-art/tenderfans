    ...
    <section className="profile-page">
      <div className="shell profile-shell">
        ...
        <div className="venue-profile-copy">
          ...
          <div className="venue-contact"><div><strong>Address</strong><span>{venue.street_address}<br />{venue.city}, {venue.state_region} {venue.postal_code}</span></div>{venue.public_phone && <div><strong>Phone</strong><a href={`tel:${venue.public_phone}`}>{venue.public_phone}</a></div>}{venue.website_url && <div><strong>Website</strong><a href={venue.website_url} target="_blank" rel="noopener noreferrer">Visit website</a></div>}<div><strong>Hours</strong><span>{Array.isArray(venue.regular_hours) && venue.regular_hours.length ? venue.regular_hours.map((day: string) => <span key={day} style={{display:"block"}}>{day}</span>) : "Hours coming soon"}</span></div></div>
        </div>
        ...
        <section className="future-strip full">
