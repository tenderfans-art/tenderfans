-- ============================================================
-- 023 CONTEST SHOUT SUBMISSION
--
-- Server-only RPC:
--   * requires active contest
--   * enforces phone identity x Tender x 7-day cooldown
--   * validates Tender / Spot relationship
--   * creates normal published Shout
--   * records qualifying contest entry
--
-- Same verified phone may immediately Shout a different Tender.
-- ============================================================
...
begin
  ...
  -- Contest must be active and currently within its date window.
  if not exists (
    select 1
  ...
  -- Enforce one qualifying Shout per verified phone,
  -- per Tender, every seven days.
  select e.created_at
  into v_recent_entry
  ...
  -- Create the normal TenderFans Shout.
  ...
  -- Attach only valid active bartender traits.
  ...
  -- Require every submitted trait to resolve successfully.
  if (
    select count(*)
...
-- This function must never be callable directly from the browser.
revoke all
