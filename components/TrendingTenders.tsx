...
export default async function TrendingTenders() {
  ...
  const bartenderTraitCounts = new Map<
    string,
    Map<number, number>
  >();
  ...
  for (const row of shoutTraitRows ?? []) {
    ...
    const counts =
      bartenderTraitCounts.get(bartenderId)!;
    ...
    (bartenderRows ?? [])
      .map((bartender: any) => {
        ...
        return {
          ...
          venueName:
            primaryVenue?.name ?? "TenderFans",
          city: primaryVenue?.city ?? "",
          venues: tenderVenues,
          cheers:
            shoutCounts.get(bartender.id) ?? 0,
          claimed: false,
          photo: photoMap.get(bartender.id),
          tags: [
            ...(
              bartenderTraitCounts
                .get(bartender.id)
                ?.entries() ?? []
            ),
          ]
      ...
      <div className="shell">
        <div className="section-title">
          <div>
            ...
            <h2>
              People worth knowing behind the bar.
            </h2>
          </div>
